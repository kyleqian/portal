...
import { Commitment, SystemProgram } from '@solana/web3.js';
...
export const CANDY_MACHINE_INSTANCE = new anchor.web3.PublicKey(
  process.env.NEXT_PUBLIC_NETWORK === 'mainnet-beta'
    ? '9uoLmvTJDMVQdJacuCpNexFSofmnwASBPeboKV4Tvr2z' // WL2 + the rest
    : 'D3yYEWLByQy6dUbw4wEWbA69VCrCxDqUaAbhNQLr8b37'
);
...
const FREEZE_AUTHORITY = new anchor.web3.PublicKey(
  process.env.NEXT_PUBLIC_NETWORK === 'mainnet-beta'
  ...
  instructions.push(
    candyMachine.program.instruction.mintNft(creatorBump, {
      accounts: {
        ...
        freezeAuthority: FREEZE_AUTHORITY, // SS ADDED
        metadataAuthority: UPDATE_AUTHORITY, // SS ADDED
...
export const shortenAddress = (address: string, chars = 6, showDots = true): string => {
  return `${address.slice(0, chars)}${showDots ? '...' : ''}`;
};
